...
int asciiCount [THREAD_NUM][STANDARD_ASCII];
...
void *runner(void *param)
{
    ...
    //walk over all ascii chars and add them to 2d array
    for(int i = 0; (i < section.end) && (run_index < BUFFER_SIZE); i++)
    {
        asciiChar = buffer[run_index++];
        asciiCount[section.index][asciiChar]++;
    }
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < STANDARD_ASCII; i++)
    {
        ...
        {
            count += asciiCount[j][i];
        }

        printf("%i occurrences of ", count);
